...
def includeme(config, plugin_config=None):
    ...
    if plugin_config and plugin_config.get('plugins'):
        for name in plugin_config['plugins']:
            configure_plugins(
                config, {name: plugin_config['plugins'][name]}, 'openprocurement.api.plugins', name
            )

    LOGGER.info("Included openprocurement.api plugin", extra={'MESSAGE_ID': 'included_plugin'})
