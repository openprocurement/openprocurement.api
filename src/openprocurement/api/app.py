...
def _search_for_plugins(config, settings):
    ...
    _plugin(config, plugins, 'openprocurement.api.plugins')
    return plugins
...
def _search_subscribers(config, settings):
    subscribers_keys = [k for k in settings if k.startswith('subscribers.')]
    subscribers = [settings[k].split(',') for k in subscribers_keys]
    for subscriber, key in zip(subscribers, subscribers_keys):
        _subscriber_plugin(subscriber, config, key)
...
def main(_, **settings):
    ...
    if not os.environ.get('MIGRATION_SKIP'):
        _plugin(config, plugins, 'openprocurement.api.migrations')
    config.registry.server_id = settings.get('id', '')
    _search_subscribers(config, settings)
