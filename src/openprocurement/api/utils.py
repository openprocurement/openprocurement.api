...
def calculate_business_date(date_obj, timedelta_obj, context=None, working_days=False):
    ...
    if context and 'procurementMethodDetails' in context and context['procurementMethodDetails']:
        ...
        if re_obj and 'accelerator' in re_obj.groupdict():
            calculated_date = date_obj + (timedelta_obj / int(re_obj.groupdict()['accelerator']))
    if calculated_date is None and working_days:
    ...
    calculated_date = date_obj + timedelta_obj if calculated_date is None else calculated_date
    calculated_date = datetime(
        calculated_date.year,
        calculated_date.month,
        calculated_date.day,
        calculated_date.hour,
        calculated_date.minute,
        calculated_date.second,
        calculated_date.microsecond,
    )
    return TZ.localize(calculated_date)
