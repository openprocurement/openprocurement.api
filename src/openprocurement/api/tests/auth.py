...
class TestIniTypeLoader(unittest.TestCase):
    ...
    def test_empty_config(self):
        ...
        result = ini_auth(self.app_meta)
        self.assertEqual(result, {})

        # Check the fact that get_path_to_auth called with app_meta
        self.assertEqual(
            self.mock_get_path.call_count,
            1,
            'Path to file was not generated with get_path_to_auth function'
        )
        self.mock_get_path.assert_called_with(self.app_meta)

        self.assertEqual(self.mock_config_parser_class.call_count, 1, 'Ini parser was not initialized')

        # Check the fact that parser search for file with path generated by get_path_to_auth
        self.assertEqual(self.mock_parser.read.call_count, 1, 'Parser doesn`t read config file')
        self.mock_parser.read.assert_called_with(self.path)

        self.assertEqual(self.mock_parser.sections.call_count, 2)

        # If config empty this logger message appear
        self.mock_logger.warning.assert_called_with(
            "Auth file '%s' was empty, no user will be added", self.path
        )

        # If config is empty create_users_from_group_ini must not be called
        self.assertEqual(
            self.mock_create_users_ini.call_count,
            0,
            'create_users_from_group was called but config empty'
        )
    ...
    def test_ini_type(self):
        ...
        # If config is not empty warning shouldn't appear
        self.assertEqual(self.mock_logger.warning.call_count, 0, 'No warning should be when config is not empty')

        # Check the fact that create_users_from_group_ini was called as many as sections in config
        # and second was with last group as a parameter
        self.assertEqual(
            self.mock_create_users_ini.call_count,
            2,
            'create_users_from_group_ini was not called as many as sections in config'
        )
...
class TestYamlTypeLoader(unittest.TestCase):
    ...
    def test_empty_config(self):
        ...
        result = yaml_auth(self.app_meta)
        self.assertEqual(result, {})
        ...
        # Check the fact that file was opened with path generated by get_path_to_auth
        self.assertEqual(self.mock_open.call_count, 1, 'File was not opened')
        self.mock_open.assert_called_with(self.path)

        self.assertEqual(self.mock_yaml.safe_load.call_count, 1, 'File was not converted to dict from yaml')
        self.mock_yaml.safe_load.assert_called_with(self.auth_file)

        # If config is empty this logger message appear
        self.assertEqual(self.mock_logger.warning.call_count, 1, 'Config is not empty')
        self.mock_logger.warning.assert_called_with(
            "Auth file '%s' was empty, no user will be added", self.path
        )

        # If config is empty create_users_from_group must not be called
        self.assertEqual(self.mock_create_users.call_count, 0, 'create_users_from_group was called but config empty')
    ...
    def test_yaml_config(self):
        ...
        # If config is not empty warning shouldn't appear
        self.assertEqual(self.mock_logger.warning.call_count, 0, 'Config is empty')

        # Check the fact that create_users_from_group was called as many as groups in config
        # and second was with group2 as a parameter
        self.assertEqual(
            self.mock_create_users.call_count,
            2,
            'create_users_from_group was not called as many as sections in config'
        )
...
class TestJsonTypeLoader(unittest.TestCase):
    ...
    def test_empty_config(self):
        ...
        result = json_auth(self.app_meta)
        self.assertEqual(result, {})
        ...
        self.assertEqual(
            self.mock_json.load.call_count,
            1,
            'File was not converted to dict from json'
        )
        self.mock_json.load.assert_called_with(self.auth_file)

        # If config is empty this logger message appear
        self.assertEqual(
            self.mock_logger.warning.call_count,
            1,
            'Config is not empty'
        )
        self.mock_logger.warning.assert_called_with(
